...
We analysed a gene expression dataset from the Norwegian Women and Cancer
(NOWAC) study. The dataset contains ... 
% (words from Vanessa's paper, more to
come) 

% BF: Hei LA var dette noe sånn du så for deg? 
We use R and various packages from Bioconductor and CRAN to analyse gene
expression datasets. From the initial analyses we built an R package that makes
it possible to share results between researchers within, and outside our group.
To build an application on top of the analyses there are two possibilities,
either develop a script that generates a static output file (e.g. generate a
html report or CSV files that contain the result data) build an application that
uses this static data, or use a framework such as Shiny or OpenCPU to create a
dynamic application that interface directly with the R code. To update an
application building on top of a static output file requires manual execution of
the output script, while using Shiny or OpenCPU alleviates this task, 
