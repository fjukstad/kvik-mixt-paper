...
% LAB: med collection tenker jeg på å laste ned ting fra nettet (slik de gjør det i Princeton) vs produksjon/ generering som er å lage ny data
...
% LAB: novel methods -> better tools (for det er vel det Kvik løser; enklere å lage verktøy som faktisk er nyttig for de som analyserer data)
...
% LAB: synes dette er for lite fokusert, og at fokus bør være å gjøre det enkelt å bygge spesialiserte verktøy, med god peformance, og enkel maintainance. Dette gjør målet litt smalere og enklere å oppnå. Jeg mener også at en av F1000 reviewerene likte denne fokusen veldig godt.
...
% LAB: Siden dette er en ikke-bioinformatikk konferanse kan det gis ett eksempel på hvorfor dette er nødvendig. 
...
% LAB: Ville snudd på denne; Motivasjon først, og "argumentet" sist
...
% LAB: I mange beskrives løsningen før kravet. Så kan si at dette er features.
\begin{enumerate}
    ...
    \item It has a low latency language-independent interface to online
        % LAB: behovet for provenance er ikke nevnt før
        reference databases in biology that captures provenance of the
        meta-data. 
    % LAB: behovet er ikke motivert for disse tre.
...
% LAB: Blir litt rart å beskrive dette etter at kravene er listet opp. Så første setning kan flyttes høyere opp.
% LAB: Og "The key services" kan flettes inn i neste paragraf.
